...
public sealed class RespawnWorker : BackgroundService
{
    ...
    private CancellationTokenSource _settingsChangedCts = new();
    ...
    {
        var old = Interlocked.Exchange(ref _settingsChangedCts, new CancellationTokenSource());
        old.Cancel();
        old.Dispose();
        _logger.LogInformation("[RespawnWorker] 🔄 Settings changed, interrupting wait to recompute");
    }
    ...
    protected override async Task ExecuteAsync(CancellationToken stoppingToken)
    {
        ...
        try
        {
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                using var linkedCts = CancellationTokenSource.CreateLinkedTokenSource(stoppingToken, _settingsChangedCts.Token);

                try
                {
                    await Task.Delay(nextDelay, linkedCts.Token);
                }
                catch (OperationCanceledException) when (!stoppingToken.IsCancellationRequested)
                {
                    // Settings changed mid-wait — loop back and recompute next10/next2 fresh
                    continue;
                }
