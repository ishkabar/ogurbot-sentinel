...
private void AdjustFontSizes(double availableWidth, double availableHeight)
{
    ...
    private void UpdateCountdowns()
    {
        ...
        if (_next10m.HasValue)
        {
            var remaining = _next10m.Value - now;
        ...
        if (_next2h.HasValue)
        {
            var remaining = _next2h.Value - now;
    ...
    private async Task SyncWithApi()
    {
        try
        {
            ...
            if (times != null)
            {
                // ✅ Zastosuj offset czasu (timer_displayed = timer_received - offset)
                // Np. jeśli offset = -30, to pokazujemy 30 sekund mniej (timer szybciej dochodzi do 0)
                _next10m = times.Next10m.ToLocalTime().AddSeconds(-_settings.TimeOffsetSeconds);
                _next2h = times.Next2h.ToLocalTime().AddSeconds(-_settings.TimeOffsetSeconds);
                
                NextTime10mText.Text = $"Next: {_next10m.Value:HH:mm:ss}";
                NextTime2hText.Text = $"Next: {_next2h.Value:HH:mm:ss}";
